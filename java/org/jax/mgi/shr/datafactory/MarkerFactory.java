...
public class MarkerFactory
{
    ...
    */
    ...
    /** retrieve any GO notes for the marker with the given 'key'.
    * @param key the marker key of the marker whose data we seek
    * @return DTO with the DTOConstants.Notes field mapped to a String of
    *    notes.  If there are no notes for the marker with the given
    *    'key', returns an empty DTO.
    ...
    *    stepping through the results
    */
    public DTO getGONotes (int key) throws DBException
    {
        ResultsNavigator nav = null;                // set of query results
        RowReference rr = null;                     // one row in 'nav'
        StringBuffer notes = new StringBuffer();    // marker notes so far
        ...
        nav = this.sqlDM.executeQuery (Sprintf.sprintf (GO_NOTES, key));
        while (nav.next())
        {
            rr = (RowReference) nav.getCurrent();
            notes.append (rr.getString(1));
    ...
    */
    public DTO getGOAnnotations (int key) throws DBException
    {
        ...
        ArrayList jnumIDs  = null;      // reference keys for the current term
        ...
        String  jnumID  = null;         // jnum of reference
        ...
        while (nav.next())
        {
            ...
            if (!evidence.containsKey(evidenceLookup))
            {
                ...
                jnumIDs = new ArrayList();        // build 1-item list of refs
                jnumIDs.add (jnumID);
                ...
                evidenceDTO.set (DTOConstants.Jnum, jnumIDs);
            ...
            else
            {
                ...
                for (int i = 0; i < evidenceCount; i++)
                {
                    ...
                    if (evidenceMatch && inferredMatch)
                    {
                        ...
                        jnumIDs = (ArrayList)
                            evidenceDTO.get(DTOConstants.Jnum);
...
*/
    ...
    */
    ...
    // get GO note chunks for given marker
    // fill in: marker key (int)
    private static final String GO_NOTES =
                "select mnc.note "
                + "from MGI_Note mn, MGI_NoteChunk mnc "
                + "where mn._Note_key = mnc._Note_key "
                + "  and mn._Object_key = %d "
                + "  and mn._NoteType_key = 1014 "
