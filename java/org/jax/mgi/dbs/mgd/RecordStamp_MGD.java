...
import org.jax.mgi.dbs.mgd.dao.UserLookup;
...
public class RecordStamp_MGD implements RecordStamp
{
  ...
  {
    if (user == null)  // the User has not been found yet
    {
      ...
      try {
        ...
        steamName = cfgReader.getJobStream();
        lookup = new UserLookup();
        user = lookup.lookupByName(steamName);
      }
      catch (MGIException e) {
        BCPExceptionFactory eFactory = new BCPExceptionFactory();
        BCPException e2 = (BCPException)
            eFactory.getException(BCPExceptionFactory.UserNotFound, e);
        e2.bind(steamName);
        throw e2;
      }
      if (user == null) {
    ...
    String key = Converter.toString(user.getKey().getKey());
    //String key = null;
